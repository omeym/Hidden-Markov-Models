...
class HMM:
    ...
    def sequence_prob(self, Osequence):
        ...
        prob = np.sum(np.multiply(alpha,beta), axis =0)[0]
    ...
    def posterior_prob(self, Osequence):
        ...
        normalization_factor = self.sequence_prob(Osequence)
        gamma = np.multiply(alpha,beta)/normalization_factor
    ...
    def likelihood_prob(self, Osequence):
        ...
        for t in range(L-1):
            for s in range(S):
                prob[s,:,t] = alpha[s,t]*self.A[s,:]*self.B[:,O[t+1]]*beta[:,t+1]/normalization_factor
    ...
    def viterbi(self, Osequence):
        ...
        for t in range(1,L):
            for s in range(S):
                delta[s,t] = self.B[s,O[t]]*np.max(np.multiply(self.A[:,s],delta[:,t-1]), axis = 0)
                Delta[s,t] = np.argmax(np.multiply(self.A[:,s],delta[:,t-1]))
        
        #Backtracking
        path.append(self.find_key(self.state_dict,np.argmax(delta[:,L-1])))
        z = np.argmax(delta[:,L-1])
        for t in range(L-1,0,-1):
            if(t is not L-1):
                z = index
            index = Delta[z,t]
            path.append(self.find_key(self.state_dict, index))
